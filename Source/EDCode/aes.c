...
void aes128_ecb_decrypt(const uint8_t* input, int base_len, const uint8_t* key, uint8_t *output)
{
...
void aes128_ecb_encrypt_hex(const uint8_t* input, const uint8_t* key, int flag, uint8_t *output)
{
    uint8_t mid_str_hex[256]; // aes加密解密的字符串不能大于256字节
    int decrypt_str_len = 0;
    memset(mid_str_hex, 0, sizeof(mid_str_hex));
    // 加密
    decrypt_str_len = aes128_ecb_encrypt(input, key, mid_str_hex);
    // 转为16进制输出
//    ascii2hex(mid_str_hex, decrypt_str_len, flag, output);
}
